...
public class ApplianceControlServer {
    ...
    public void start() throws IOException, InterruptedException {
        grpcServer = ServerBuilder.forPort(PORT)
                .addService(ServerInterceptors.intercept(new ApplianceControlServiceImpl(), new AuthInterceptor()))
                .build().start();
        logger.info("ApplianceControlServer started on port " + PORT);
        jmdns = JmDNS.create(InetAddress.getLocalHost());
        jmdns.registerService(ServiceInfo.create(SERVICE_TYPE, SERVICE_NAME, PORT, "Smart Home Appliance Control Service"));
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        new ApplianceControlServer().start();
    }
}
